 ...
 */
export function extractAccountData(data: GraphQLContributorData, analysisWindowMonths: number): AccountData {
  ...
  const consistencyScore = totalMonthsInWindow > 0 ? activeMonths / totalMonthsInWindow : 0
 ...
 */
export function checkAccountAge(data: AccountData, threshold: number): MetricCheckResult {
  ...
  details += passed ? ` (meets threshold >= ${threshold} days)` : ` (below threshold >= ${threshold} days)`
 ...
 * @returns MetricCheckResult with pass/fail status
 */
export function checkActivityConsistency(data: AccountData, threshold: number): MetricCheckResult {
  ...
  details += passed ? ` (meets threshold >= ${thresholdPercent}%)` : ` (below threshold >= ${thresholdPercent}%)`
 ...
 */
export function isNewAccount(data: AccountData, thresholdDays: number): boolean {
