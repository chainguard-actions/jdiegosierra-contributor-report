 ...
 */
export function extractPRHistoryData(data: GraphQLContributorData, sinceDate: Date): PRHistoryData {
  ...
  const closedWithoutMerge = prs.filter((pr) => pr.state === 'CLOSED' && !pr.merged)
  ...
  const veryShortPRs = prs.filter((pr) => pr.additions + pr.deletions < VERY_SHORT_PR_THRESHOLD).length

  const mergedPRDates = mergedPRs.filter((pr) => pr.mergedAt).map((pr) => new Date(pr.mergedAt!))
 ...
 */
export function checkPRMergeRate(data: PRHistoryData, threshold: number): MetricCheckResult {
