...
import type { RepoQualityData, RepoContribution, MetricCheckResult } from '../types/metrics.js'
...
export function extractRepoQualityData(
  ...
  const averageRepoStars = contributedRepos.length > 0 ? totalStars / contributedRepos.length : 0

  const highestStarRepo = contributedRepos.length > 0 ? Math.max(...contributedRepos.map((r) => r.stars)) : 0
 ...
 */
export function checkRepoQuality(data: RepoQualityData, threshold: number, minimumStars: number): MetricCheckResult {
